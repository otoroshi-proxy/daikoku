...
```sbt
~run -Ddaikoku.tenants.provider=hostname -Ddaikoku.mode=dev -Ddaikoku.postgres.database=daikoku -Ddaikoku.exposedOn=5173 -Ddaikoku.otoroshi.sync.cron=true -Ddaikoku.init.data.from=$DAIKOKU_PROJECT_ROOT/daikoku/javascript/tests/config/daikoku/daikoku_state_very_local.ndjson -Ddaikoku.health.accessKey=secret
```
...
```bash
docker exec -it postgres_daikoku psql -U daikoku -d daikoku -c '\dt'
```
